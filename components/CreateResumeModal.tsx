  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.extractedText) {
        // Simple heuristics – you can replace with AI later
        const lines = data.extractedText.split('\n');
        const nameMatch = lines.find(l => l.match(/^[A-Z][a-z]+ [A-Z][a-z]+/));
        const jobMatch = lines.find(l => l.match(/(Engineer|Manager|Developer|Analyst|Specialist|Consultant)/i));
        const skillsMatch = lines.find(l => l.toLowerCase().includes('skill') && l.length < 200);
        const summaryText = data.extractedText.split(' ').slice(0, 150).join(' ');
        
        const extractedData = {
          fullName: nameMatch ? nameMatch.trim() : '',
          jobTitle: jobMatch ? jobMatch.trim() : '',
          skills: skillsMatch ? skillsMatch.replace(/skills?:/i, '').trim() : '',
          summary: summaryText + (summaryText.length > 100 ? '...' : '')
        };
        sessionStorage.setItem('importedResume', JSON.stringify(extractedData));
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-center">Create New Resume</DialogTitle>
        ...
        <div className="grid gap-4 py-4">
          ...
          >
            ...
            <span className="text-xs opacity-90">Build a fresh resume with AI assistance</span>
          </Button>
          ...
          >
            ...
            <span className="text-lg font-semibold">Import from existing resume</span>
            <span className="text-xs text-gray-500">Upload PDF or DOCX – we’ll extract your data</span>
          </Button>
        </div>
      </DialogContent>
    </Dialog>
